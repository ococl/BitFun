...
pub(in crate::agentic::tools) struct ProductToolMaterializer;

impl ProductToolMaterializer {
    pub(in crate::agentic::tools) fn materialize_provider_groups(
        &self,
        plan: &[ToolProviderGroupPlan],
    ) -> Vec<StaticToolProviderGroup<dyn Tool>> {
        plan.iter()
            .map(|group| {
                StaticToolProviderGroup::new(
                    group.provider_id(),
                    self.materialize_tools(group.tool_names()),
                )
            })
            .collect()
    }

    fn materialize_tools(&self, tool_names: &[&str]) -> Vec<Arc<dyn Tool>> {
        tool_names
            .iter()
            .map(|tool_name| self.materialize_tool(tool_name))
            .collect()
    }

    fn materialize_tool(&self, tool_name: &str) -> Arc<dyn Tool> {
        match tool_name {
