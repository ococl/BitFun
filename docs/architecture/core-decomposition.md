...
- 后续迁移必须先按风险分层处理：
  ...
  - 2026-05-18 product-domain readiness update: MiniApp draft DTO/response shape,
    ...
    preparation now live in `bitfun-product-domains`. Core still owns MiniApp
    filesystem IO, worker process execution, built-in asset seeding/source-hash lookup,
